        (function () {
            ...
            // ==========================================================================
            // Palette — carried over from the original 2D mockup
            // ==========================================================================
            const Palette = {
                woodShadow: 0x241609, woodMid: 0x5a3a20, woodHi: 0x8a5f34, woodDark: 0x3a2410,
                brass: 0xc9a876, paper: 0xe9dfc4, sea: 0x4a6f78, seaDeep: 0x1c343d,
                skyTop: 0x6f8fa3, skyHorizon: 0xf0c583, sun: 0xffdca0, sealRed: 0x7c2b26,
                awningCream: 0xe4d3ad, dockPlank: 0x8a7658, dockDark: 0x6b5a41, rope: 0xa88a5f
            };
            ...
            // Simple orbit-style camera: yaw/pitch/dist around a movable target.
            // (Custom-written rather than the OrbitControls addon, so this file has
            // zero dependencies beyond the single three.min.js script tag — that
            // matters on the department server, which will not fetch extra JS
            // modules for you at runtime unless you bundle or CDN-link them yourself.)
            let camYaw = 0.15, camPitch = 0.3, camDist = 20;
            const camTarget = new THREE.Vector3(0, 2, 1.5);

            function updateCamera() {
                camPitch = Math.max(0.05, Math.min(1.3, camPitch));
                camDist = Math.max(6, Math.min(45, camDist));
                const x = camTarget.x + camDist * Math.sin(camYaw) * Math.cos(camPitch);
                const y = camTarget.y + camDist * Math.sin(camPitch);
                const z = camTarget.z + camDist * Math.cos(camYaw) * Math.cos(camPitch);
            ...
            // ==========================================================================
            // Shopkeeper (first-person) camera — placed later once the shop group
            // exists; declared here so the control handlers can reference it.
            // ==========================================================================
            let shopCam;            // created after shop is built
            let shopCamYaw = Math.PI;   // look toward local +Z (out the front)
            let shopCamPitch = -0.1;    // slight downward tilt
            let useShopCam = false;
            let activeCamera = camera;

            const infoEl = document.getElementById('info');
            const toggleBtn = document.getElementById('cam-toggle');
            toggleBtn.addEventListener('click', () => {
                useShopCam = !useShopCam;
                activeCamera = useShopCam ? shopCam : camera;
                toggleBtn.textContent = useShopCam ? '🌍 Orbit View' : '🏪 Shopkeeper View';
                infoEl.textContent = useShopCam
                    ? 'Drag to look around from behind the counter'
                    : 'Drag to orbit · Scroll to zoom · WASD / Arrow keys to move';
            });
            ...
            window.addEventListener('pointermove', e => {
                ...
                if (useShopCam) {
                    // Free-look from inside the shop
                    shopCamYaw -= dx * 0.004;
                    shopCamPitch -= dy * 0.004;
                    shopCamPitch = Math.max(-1.0, Math.min(0.8, shopCamPitch));
                    updateShopCam();
                } else {
                    camYaw -= dx * 0.005;
                    camPitch += dy * 0.005;
            ...
            function updateShopCam() {
                if (!shopCam) return;
                // Compute look direction from yaw/pitch in the shop's local space
                const lx = Math.sin(shopCamYaw) * Math.cos(shopCamPitch);
                const ly = Math.sin(shopCamPitch);
                const lz = Math.cos(shopCamYaw) * Math.cos(shopCamPitch);
                // lookAt needs a world-space target, but since shopCam is a child of
                // shop, we transform the local look-target through the shop's matrix.
                const localTarget = new THREE.Vector3(
                    shopCam.position.x + lx,
                    shopCam.position.y + ly,
                    shopCam.position.z + lz
                );
                const worldTarget = localTarget.clone();
                shop.localToWorld(worldTarget);
                shop.updateMatrixWorld(true);
                shopCam.lookAt(worldTarget);
            }

            // ==========================================================================
            // Lighting — hemisphere fill + directional "sun" (shadow-casting) + a
            // point light that lives inside the lantern group further down, so it
            // travels with the desk exactly the way the brief's Section 2 describes
            // ("attach lights ... as children of objects so that they move together").
            // ==========================================================================
            const ambient = new THREE.HemisphereLight(0x9fb8c9, 0x3a2f22, 0.65);
            scene.add(ambient);

            const SUN_DIR = new THREE.Vector3(0.34, 0.65, -1).normalize();
            const sun = new THREE.DirectionalLight(0xffe6b8, 2.3);
            sun.position.copy(SUN_DIR).multiplyScalar(28);
            sun.castShadow = true;
            ...
            // ==========================================================================
            // Sky — custom shader, dynamic gradient dome with a soft sun glow
            // (fulfils the "static and dynamic skyboxes" 3D-effect requirement)
            // ==========================================================================
    ...
    `,
      ...
      void main() {
        ...
        float sunAmt = pow(max(dot(normalize(vPos), uSunDir), 0.0), 24.0);
        col += uSunColor * sunAmt * 0.8;
    ...
    `
            ...
            // ==========================================================================
            // Ocean — custom shader, vertex-displaced waves + fresnel-ish specular.
            // This is the project's required custom shader: both stages used
            // purposefully, uniforms (uTime) driven by game state so the effect is
            // alive rather than static, achieving something built-in materials can't.
            // ==========================================================================
            const oceanGeo = new THREE.PlaneGeometry(160, 160, 128, 128);
            ...
            const oceanMat = new THREE.ShaderMaterial({
                uniforms: {
                    ...
                    uWaveScale: { value: 1 },
                    uWaveSpeed: { value: 1 },
                    uShallow: { value: new THREE.Color(Palette.sea) },
      ...
      void main() {
        ...
        vHeight = h;
        vWorldPos = p; // Plane is at origin, so local == world
    ...
    `
            ...
            gulls.forEach(g => scene.add(g.mesh));

            // ==========================================================================
            // Debug tweak panel (lil-gui) — drag values to adjust the scene live,
            // no editor, no reload. Hit "Log values to console" to print the
            // current numbers as ready-to-paste code, then bake them into the
            // scene-building code above. This whole block is a build-time tool,
            // not gameplay — delete it (and the lil-gui script tag) before your
            // final submission.
            // ==========================================================================
            const gui = new lil.GUI({ title: 'Scene tweaks' });

            const shopFolder = gui.addFolder('Shop');
            shopFolder.add(shop.position, 'x', -15, 15, 0.1).name('position x');
            shopFolder.add(shop.position, 'z', -12, 12, 0.1).name('position z');
            const shopRotationDeg = { y: THREE.MathUtils.radToDeg(shop.rotation.y) };
            shopFolder.add(shopRotationDeg, 'y', -180, 180, 1).name('rotation y (deg)')
                .onChange(v => shop.rotation.y = THREE.MathUtils.degToRad(v));

            const waveFolder = gui.addFolder('Ocean waves');
            waveFolder.add(oceanMat.uniforms.uWaveScale, 'value', 0, 3, 0.05).name('height');
            waveFolder.add(oceanMat.uniforms.uWaveSpeed, 'value', 0, 3, 0.05).name('speed');

            const sunFolder = gui.addFolder('Sun');
            const sunDirCtrl = { x: SUN_DIR.x, y: SUN_DIR.y, z: SUN_DIR.z };
            function applySunDir() {
                SUN_DIR.set(sunDirCtrl.x, sunDirCtrl.y, sunDirCtrl.z).normalize();
                sun.position.copy(SUN_DIR).multiplyScalar(28);
            }
            sunFolder.add(sunDirCtrl, 'x', -1, 1, 0.01).onChange(applySunDir).name('direction x');
            sunFolder.add(sunDirCtrl, 'y', 0.05, 1, 0.01).onChange(applySunDir).name('direction y');
            sunFolder.add(sunDirCtrl, 'z', -1, 1, 0.01).onChange(applySunDir).name('direction z');
            sunFolder.add(sun, 'intensity', 0, 3, 0.05).name('intensity');

            const lanternFolder = gui.addFolder('Lantern light');
            lanternFolder.add(lanternLight, 'intensity', 0, 3, 0.05);
            lanternFolder.addColor({ color: '#' + lanternLight.color.getHexString() }, 'color')
                .onChange(hex => lanternLight.color.set(hex));

            gui.add({
                logValues: () => {
                    console.log(
                        `shop.position.set(${shop.position.x.toFixed(2)}, ${shop.position.y.toFixed(2)}, ${shop.position.z.toFixed(2)});\n` +
                        `shop.rotation.y = ${shop.rotation.y.toFixed(3)}; // ${shopRotationDeg.y.toFixed(0)}deg\n` +
                        `SUN_DIR.set(${SUN_DIR.x.toFixed(3)}, ${SUN_DIR.y.toFixed(3)}, ${SUN_DIR.z.toFixed(3)}).normalize();\n` +
                        `sun.intensity = ${sun.intensity.toFixed(2)};\n` +
                        `oceanMat.uniforms.uWaveScale.value = ${oceanMat.uniforms.uWaveScale.value.toFixed(2)};\n` +
                        `oceanMat.uniforms.uWaveSpeed.value = ${oceanMat.uniforms.uWaveSpeed.value.toFixed(2)};\n` +
                        `lanternLight.intensity = ${lanternLight.intensity.toFixed(2)};\n` +
                        `lanternLight.color.set(0x${lanternLight.color.getHexString()});`
                    );
                }
            }, 'logValues').name('Log values to console');

            // ==========================================================================
            // Create the shopkeeper camera now that the shop group exists
            // ==========================================================================
            shopCam = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 0.1, 500);
            shopCam.position.set(0, 3.6, -1.2);  // local: behind counter, standing eye height
            shop.add(shopCam);
            updateShopCam();
            ...
            window.addEventListener('resize', () => {
                ...
                shopCam.aspect = aspect;
                shopCam.updateProjectionMatrix();
                renderer.setSize(window.innerWidth, window.innerHeight);
            });
            ...
            // ==========================================================================
            let weatherConfigs = null;

            async function loadLevel(dayNum) {
                try {
                    if (!weatherConfigs) {
                        const res = await fetch('/src/assets/levels/weather.json');
                        weatherConfigs = await res.json();
                    }
                    const levelRes = await fetch(`/src/assets/levels/day${dayNum}.json`);
                    const levelData = await levelRes.json();
                    
                    const weatherState = weatherConfigs[levelData.weather];
                    if (weatherState) {
            ...
            function applyWeather(w) {
                ...
                SUN_DIR.set(w.sunDir.x, w.sunDir.y, w.sunDir.z).normalize();
                sun.position.copy(SUN_DIR).multiplyScalar(28);
                
                skyMat.uniforms.uTop.value.set(w.skyTop);
                skyMat.uniforms.uHorizon.value.set(w.skyHorizon);
                skyMat.uniforms.uSunColor.value.set(w.sunColor);
                
                oceanMat.uniforms.uDeep.value.set(w.seaDeep);
                oceanMat.uniforms.uShallow.value.set(w.seaShallow);
                oceanMat.uniforms.uWaveScale.value = w.waveScale;
                oceanMat.uniforms.uWaveSpeed.value = w.waveSpeed;
            ...
            function animate() {
                ...
                if (!useShopCam) {
                    const panSpeed = 0.12;
                    if (keys['arrowup'] || keys['w']) camTarget.z -= panSpeed;
                    if (keys['arrowdown'] || keys['s']) camTarget.z += panSpeed;
                    if (keys['arrowleft'] || keys['a']) camTarget.x -= panSpeed;
                    if (keys['arrowright'] || keys['d']) camTarget.x += panSpeed;
                    updateCamera();
                }

                signPivot.rotation.z = Math.sin(t * (2 * Math.PI / 5)) * 0.045;

                gulls.forEach(g => {
                    g.mesh.position.set(
                        g.baseX + Math.sin(t * 0.4 + g.phase) * 3,
                        g.baseY + Math.sin(t * 1.1 + g.phase) * 0.5,
                        -6 + Math.cos(t * 0.3 + g.phase) * 2
                    );
                });
